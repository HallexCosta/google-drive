...
describe("#UploadHandler test suite", () => {
  ...
  describe("#handleFileBytes", () => {
    test("should call emit function and it is a transform stream", async () => {
      ...
      await pipeline(source, handler.handleFileBytes(filename), target);

      expect(ioObj.to).toHaveBeenCalledTimes(messages.length);
      expect(ioObj.emit).toHaveBeenCalledTimes(messages.length);

      // se o handleFileBytes for um tranform stream, nosso pipeline
    ...
    test("given message timerDelay as 2secs it should emit only to messages during 2 seconds period", async () => {
      jest.spyOn(ioObj, ioObj.emit.name);
